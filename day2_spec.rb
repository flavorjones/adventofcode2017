...
class SpreadsheetRow
  ...
  def calculate_checksum2
    cells.each_with_index do |divisor, jdivisor|
      cells.each_with_index do |dividend, jdividend|
        next if jdivisor == jdividend
        return dividend / divisor if (dividend % divisor) == 0
      end
    end
    raise "could not find two numbers that divide evenly"
  end
end
...
class Spreadsheet
  ...
  def initialize rows
    @rows = rows.map { |row| SpreadsheetRow.new row }
  end

  def calculate_checksum
    rows.inject(0) { |sum, row| sum += row.calculate_checksum }
  end

  def calculate_checksum2
    rows.inject(0) { |sum, row| sum += row.calculate_checksum2 }
  end
end
...
describe SpreadsheetRow do
  describe "calculate_checksum" do
    it { expect(SpreadsheetRow.new([5, 1, 9, 5]).calculate_checksum).to eq(8) }
    it { expect(SpreadsheetRow.new([7, 5, 3]).calculate_checksum).to eq(4) }
    it { expect(SpreadsheetRow.new([2, 4, 6, 8]).calculate_checksum).to eq(6) }
  end

  describe "calculate_checksum2" do
    it { expect(SpreadsheetRow.new([5, 9, 2, 8]).calculate_checksum2).to eq(4) }
    it { expect(SpreadsheetRow.new([9, 4, 7, 3]).calculate_checksum2).to eq(3) }
    it { expect(SpreadsheetRow.new([3, 8, 6, 5]).calculate_checksum2).to eq(2) }
  end
end
...
describe Spreadsheet do
  describe "calculate_checksum" do
    it "does a thing" do
      ...
      expect(spreadsheet.calculate_checksum).to eq(18)
    end
  end
  ...
  describe "calculate_checksum2" do
    it "does a thing" do
      ...
      expect(spreadsheet.calculate_checksum2).to eq(9)
    end
  end
end
...
puts spreadsheet.calculate_checksum
puts spreadsheet.calculate_checksum2
